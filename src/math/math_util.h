// Copyright 2016-2017 kvedder@umass.edu, joydeepb@cs.umass.edu,
//  slane@cs.umass.edu
...
// found.
template <typename T>
unsigned int SolveCubic(const T& a,
             const T& b,
             const T& c,
             const T& d,
             T* r0,
  ...
  if (discriminant > T(0)) {
    ...
    const T q =
        (T(2) * Cube<T>(b) - 9 * a * b * c + 27 * Sq<T>(a) * d) /
        (T(27) * Cube<T>(a));
    ...
    for (T k = 0; k < 3; k++) {
      roots.push_back(
      T(2) * sqrt(-p/T(3)) * cos(T(1.0/3.0)*acos(T(3)*q/(T(2)*p)* sqrt(T(-3)/p))
      - T(2.0) * k * M_PI/T(3.0))
      - b/(T(3)*a));
    }
    ...
    *r2 = roots[0];
    ...
    if (discriminant0 == 0) {
      *r2 = -b/(T(3) * a);
      ...
      const T double_root = (T(9) * a * d - b * c) /
                            (T(2) * discriminant0);

      const T simple_root =
          (T(4) * a * b * c - 9 * Sq<T>(a) * d - Cube<T>(b)) /
          (a * discriminant0);
